...
public class Hairdryerscript : MonoBehaviour {
	...
	void Update () {
		...
		if (!trumpshootscript.shoot && playergearscript.launchernumber == 1) {
			donaldtrump.GetComponent<Rigidbody2D> ().isKinematic = true;
			donaldtrump.GetComponent<SpriteRenderer> ().enabled = false;
		} else if (trumpshootscript.shoot && playergearscript.launchernumber == 1) {
